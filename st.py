...
if uploaded_file is not None:
    ...
    for entry in bibtex_data.entries.values():
        author_field = entry.persons.get("author", [])
        
        # Full name handling for multiple authors
        if author_field:
            # Join author names with "and"
            full_author_names = [
                " ".join(person.first_names + person.last_names) for person in author_field
            ]
            for full_name in full_author_names:
